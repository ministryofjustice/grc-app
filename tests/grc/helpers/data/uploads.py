...
class UploadsHelpers:
    ...
    def get_uploads_object_data(self, extensions_and_number, medical_report_slot=None):
        uploads_file_data = []
        for extension, number in extensions_and_number.items():
            for i in range(1, number + 1):
                ...
                ef.medical_report_slot = medical_report_slot
                uploads_file_data.append(ef)
                self.aws_file_names.append(ef.aws_file_name)
        return uploads_file_data

    def get_uploads_object_data_pdf(self, number_of_files, number_passwords_required=0, medical_report_slot=None):
