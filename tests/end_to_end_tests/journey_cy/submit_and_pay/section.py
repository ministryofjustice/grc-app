...
async def run_checks_on_section(page: Page, asserts: AssertHelpers, helpers: PageHelpers):
    ...
    await asserts.check_your_answers_row(row_name='Ydych chi erioed wedi cael Tystysgrif Cydnabod Rhywedd (neu dystysgrif cyfwerth) mewn gwlad arall?', expected_value='Oes')
    await asserts.check_your_answers_row(row_name="A oes gennych chi ddogfennaeth swyddogol sy'n dangos eich bod wedi cael Tystysgrif Cydnabod Rhywedd (neu dystysgrif cyfwerth) yn un o'r gwledydd neu diriogaethau a ganiateir?", expected_value='Oes')
    await asserts.check_your_answers_row(row_name="Ydych chi'n cydsynio i'r Swyddfa Gofrestru Gyffredinol gysylltu â chi yngl?n â'ch cais?", expected_value='Oes')
    ...
    await asserts.check_your_answers_row(row_name='Erioed wedi newid enw', expected_value='Oes')
    ...
    await asserts.check_your_answers_row(row_name='Ddim ar gael yn y 6 mis nesaf', expected_value=f"Oes\n{data.DATES_TO_AVOID}")
    await asserts.check_your_answers_row(row_name='Hysbysu HMRC', expected_value='Oes')
    ...
    await asserts.check_your_answers_row(row_name='Genedigaeth a gofrestrwyd yn y DU', expected_value='Oes')
    ...
    await asserts.check_your_answers_row(row_name="Enw’r tad a restrwyd", expected_value='Oes')
    await asserts.check_your_answers_row(row_name="Enw’r tad", expected_value=f"{data.FATHERS_FIRST_NAME} {data.FATHERS_LAST_NAME}")
    await asserts.check_your_answers_row(row_name='Mabwysiadwyd', expected_value='Oes')
    await asserts.check_your_answers_row(row_name='Mabwysiadwyd yn y DU', expected_value="Dydw i ddim yn gwybod")
    await asserts.check_your_answers_row(row_name="Gwasanaeth cofrestru’r Lluoedd, Conswl Prydeinig neu Uchel Gomisiwn, neu dan ddarpariaethau Llongau Masnach neu Hedfan Sifil", expected_value='Nac oes')
    ...
    await asserts.check_your_answers_row(row_name='Mae fy mhriod neu fy mhartner wedi marw', expected_value='Oes')
    await asserts.check_your_answers_row(row_name='Mae ein priodas neu bartneriaeth sifil wedi dod i ben', expected_value='Oes')
