...
async def run_checks_on_section(page: Page, asserts: AssertHelpers, helpers: PageHelpers):
    ...
    await asserts.check_your_answers_row(row_name="Enw’r rhiant cyntaf", expected_value=f"{data.MOTHERS_FIRST_NAME} {data.MOTHERS_LAST_NAME}\n(Enw cyn priodi: {data.MOTHERS_MAIDEN_NAME})")
    await asserts.check_your_answers_row(row_name="Enw’r ail riant a restrwyd", expected_value='Ydw')
    await asserts.check_your_answers_row(row_name="Enw’r ail riant", expected_value=f"{data.FATHERS_FIRST_NAME} {data.FATHERS_LAST_NAME}")
