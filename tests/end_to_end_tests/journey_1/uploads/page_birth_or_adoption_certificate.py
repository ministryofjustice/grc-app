...
TASK_LIST_BUTTON_NAME = 'Birth or adoption certificate'
...
async def run_checks_on_page(page: Page, asserts: AssertHelpers, helpers: PageHelpers):
    ...
    # The "Save and continue" button shouldn't be visible now, because we haven't yet uploaded any documents
    await asserts.no_button('Save and continue')
    ...
    DOCUMENT_TWO_NAME = 'document_2.bmp'
    ...
    # Upload second valid document and replace first document
    await helpers.upload_file_valid(field='documents', file_name=DOCUMENT_TWO_NAME)
    page.set_default_timeout(data.TIMEOUT_FOR_SLOW_OPERATIONS)
    await helpers.click_button('Upload 1 file')
    await asserts.url(PAGE_URL)
    page.set_default_timeout(data.DEFAULT_TIMEOUT)
    ...
    await asserts.number_of_errors(0)
    await asserts.documents_uploaded(1)
    await asserts.document_uploaded(file_name=DOCUMENT_TWO_NAME)

    # Remove the uploaded document
    await helpers.click_button(f"Remove file {DOCUMENT_TWO_NAME}")
    await asserts.url(PAGE_URL)
    await asserts.accessibility()
    await asserts.h1(PAGE_H1)
    await asserts.number_of_errors(0)
